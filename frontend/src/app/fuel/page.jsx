...
export default function Page() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['fuel', pg],
    queryFn: async () => { const { data } = await api.get('/fuel?page=' + pg); return data; },
    retry: false,
  });
  const title = { projects:'Projects', fuel:'Fuel Logs', maintenance:'Maintenance', transfers:'Transfers', incidents:'Incidents' }['fuel'];
  const newHref = { projects:'/projects/new', fuel:'/fuel/new', maintenance:'/maintenance/job-cards/new', transfers:'/transfers/new', incidents:'/incidents/new' }['fuel'];
  return (
    <AppShell>
